# Modified from https://github.com/facebookresearch/co-tracker/tree/main
...
def get_3d_sincos_pos_embed_from_grid(embed_dim: int, grid: torch.Tensor) -> torch.Tensor:
    B, S, N, D = grid.shape
    assert embed_dim % 3 == 0, "embed_dim must be divisible by 3."
    dim_each = embed_dim // 3
    assert dim_each % 2 == 0, "embed_dim//3 must be even, because 1D sin/cos uses half sin, half cos."

    device = grid.device

    grid_flat = grid.reshape(B * S * N, 3)  # (M, 3)
    x = grid_flat[:, 0].reshape(1, -1).to(device)  # (1, M)
    y = grid_flat[:, 1].reshape(1, -1).to(device)
    z = grid_flat[:, 2].reshape(1, -1).to(device)

    emb_x = get_1d_sincos_pos_embed_from_grid(dim_each, x)  # (1, M, dim_each)
    emb_y = get_1d_sincos_pos_embed_from_grid(dim_each, y)  # (1, M, dim_each)
    emb_z = get_1d_sincos_pos_embed_from_grid(dim_each, z)  # (1, M, dim_each)

    emb = torch.cat([emb_x, emb_y, emb_z], dim=-1)  # (1, M, embed_dim)

    emb = emb.view(B, S, N, embed_dim)
    return emb
...
def sample_features5d(input, coords):
    """
    input:  B, T, C, H, W
    coords: B, R1, R2, 3  (t,x,y)
    return: B, R1, R2, C
    """
    B, T, C, H, W = input.shape
    # B,T,C,H,W -> B,C,T,H,W
    input = input.permute(0, 2, 1, 3, 4)
    coords = coords.unsqueeze(3)  # B,R1,R2,1,3
    feats = bilinear_sampler(input, coords)
    # feats: B,C,R1,R2,1 -> B,R1,R2,C
